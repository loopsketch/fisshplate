...
public abstract class AbstractCell implements TemplateElement {
    ...
    AbstractCell(CellWrapper cell) {
        this.cell = cell;
        Sheet templateSheet = cell.getRow().getSheet().getSheet();
        int rowNum = cell.getRow().getRow().getRowNum();
        ...
        for(int i=0; i < templateSheet.getNumMergedRegions();i++){
            ...
            setUpMergedCellInfo(cell.getCell().getColumnIndex(), rowNum, reg);
            if(isMergedCell){
                break;
            }
        }
    ...
    protected void copyCellStyle(FPContext context, Cell outCell) {
        Cell hssfCell = cell.getCell();
        CellStyle outStyle = hssfCell.getCellStyle();
    ...
    private void mergeCell(FPContext context){
        ...
        Sheet hssfSheet = cell.getRow().getSheet().getSheet();
