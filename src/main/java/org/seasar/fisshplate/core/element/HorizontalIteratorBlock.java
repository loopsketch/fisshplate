...
public class HorizontalIteratorBlock extends AbstractBlock{
    ...
    protected void mergeBlock(FPContext context, int startCell) throws FPMergeException {
        ...
        for (int i = 0; i < childList.size(); i++) {
            ...
            if(elem instanceof RowElement){
                ...
                mergeRow(context, (RowElement)elem);
    ...
    private int getMaxCellElementListSize(){
        ...
        for (int i = 0; i < childList.size(); i++) {
            TemplateElement elem = (TemplateElement) childList.get(i);
            if(elem instanceof RowElement){
                int listSize = ((RowElement)elem).getCellElementList().size();
    ...
    private void mergeRow(FPContext context, RowElement row) throws FPMergeException {
    ...
    private void mergeNoIterationBlock(FPContext context) throws FPMergeException {
        for (int i = 0; i < childList.size(); i++) {
            TemplateElement elem = (TemplateElement) childList.get(i);
            if(elem instanceof RowElement){
                mergeNoIterationRow(context, (RowElement)elem);
            }else{
                elem.merge(context);
            }
        }
    ...
    private void mergeNoIterationRow(FPContext context, RowElement row) throws FPMergeException {
