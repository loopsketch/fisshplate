...
/**
 * FiSSH Plateで{@link Workbook}を生成する際のエントリポイントとなるクラスです。
 *
 * @author rokugen
 * @author a-conv
 *
...
public class FPTemplate {
	 ...
	 */
	public Workbook process(String templateName, Map data) throws FPParseException, FPMergeException,IOException {
		InputStream is = InputStreamUtil.getResourceAsStream(templateName);
		Workbook workbook = WorkbookFactory.create(new POIFSFileSystem(is));
	 ...
	 *             ファイルIOでエラーが発生した際に投げられます。
	 */
	public Workbook process(InputStream is, Map data) throws FPParseException,FPMergeException,IOException {
		return process(WorkbookFactory.create(new POIFSFileSystem(is)), data);
	}

	/**
	 * テンプレート用ワークブックと埋め込み用データを受け取り、出力用{@link Workbook}を生成して戻します。
	 *
	 * @param hssfWorkbook
	 *            テンプレート用{@link Workbook}。
	 * @param data
	 *            埋め込み用データ
	 * @return 出力するデータ埋め込み済みの{@link Workbook}
	 * @throws FPParseException 解析時にエラーが発生した際に投げられます。
	 * @throws FPMergeException データ埋め込み時にエラーが発生した際に投げられます。
	 */
	public Workbook process(Workbook hssfWorkbook, Map data) throws FPParseException,FPMergeException {
		...
		for(int i=0; i < workbook.getSheetCount(); i++){
    		...
    		FPContext context = new FPContext(sheet.getSheet(), data);
    		// ページコンテキスト情報の追加
